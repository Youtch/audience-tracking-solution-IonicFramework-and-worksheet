...
// Sol B) découplement du code de description POUR GERER une éventuelle l'icone/image d'illustration dans le contenu 
// A TESTER
function afficherModalEtat_SolB(_titre, _iconeTitre, _contenuHtml, _iconeContenu,  _btnValider, _btnAnnuler, _btnValiderLabel, _btnAnnulerLabel)
{ // _titre:string, _iconeTitre:string, _contenuHtml:string, _iconeContenu:string, _btnAnnuler:boolean, _btnValider:boolean, _btnValiderLabel:string, _btnAnnulerLabel:string
  //alert( _titre + "\n;" +  _iconeTitre + "\n;" +_contenuHtml + "\n;" +_btnValider + "\n;" +_btnAnnuler);
  ...
  */
  ...
  if(_iconeTitre!="")
    $("#modalEtatsTrts-iconeTitre").attr("class", "fas "+ _iconeTitre ); // ou removeClass("fas").removeClass("estCache").addClass("fas "+ _iconeTitre)
// Prod :  $("#modalEtatsTrts-contenu").html( "<div style="float:left;max-width:10%">Mon petit icone mignone</div><div style="float:right;max-width:85%">"+_contenuHtml+"</div>");
//DEV test  $("#modalEtatsTrts-contenu").html( "<div style="float:left;max-width:10%">Mon petit icone mignone</div><div style="float:right;max-width:85%">"+_contenuHtml+"</div>");
//DEV test
  if(_iconeContenu!=""){
  $("#modalEtatsTrts-contenu").html(
    '<div class="container">'+
      '<div class="row">'+
        '<div class="col-2">'+ _iconeContenu +'</div>'+
        '<div class="col-sm-10">'+ _contenuHtml + '</div>'+
      '</div>'+
    '</div>'
  );
  }else{
