...
{ 
  ...
  getGlobalVars().then( oGlobalVars=>{
    //console.log("init :: oGlobalVars = ",oGlobalVars);
...
function getGlobalVars()
{
  ...
  return new Promise( resolve=>{
    getAjaxFunction("json", "http://localhost:3000/ajax/v1.7/importer/?getGlobalVars=true").then( (oGlobalVars)=>{
      if(oGlobalVars<0){ // TODO=> Cas d'erreur sur le serveur : alors "getAjaxFunction" renvoie un numéro d'erreur
        // TODO : à gérer
...
function setGlobalVars(_data)
{
  ...
  return new Promise( resolve=>{
    ...
    //Méthode B)
    setAjaxFunction("http://localhost:3000/ajax/v1.7/importer/?setGlobalVars=true&",
     [_data]
    ).then( (results)=>{
...
//      $("#btn-gnrrFchr").prop( "disabled", true );// désactiver le btn
    ...
    case 1:// Cas : enregistrer input
      ...
      if(! $("#btn-gnrrFchr").hasClass("is-invalid")){
        ...
        var result = await setGlobalVars( {key: "modeleGenerationFichierCsv", val: $("#inpt-modeleFichierCSV").val()} );
        if(result){
